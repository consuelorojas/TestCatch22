...
from classification import run_experiment, time_experiment
...
periods = [300, 600, 900, 1500, 3000, 7500, 15000]
...
samples = 100
## Output directory
sweep_name = "fhn_obs/periods"
output_dir = os.path.join("results", sweep_name)
os.makedirs(output_dir, exist_ok=True)

# Timestamped filename
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
output_file = os.path.join(output_dir, f"results_times_{timestamp}.pkl")

# Run sweep
all_results = []
for i, p in enumerate(tqdm(periods)):
    X, y = create_labeled_dataset([
        (0, 'fhn_obs', {'length':p, 'dt': 0.1, 'x0': [0,0], 'args':[b0, b1, epsilon, I, noise]}),
        (1, 'fhn_obs', {'length':p, 'dt': 0.1, 'x0': [0,0], 'args':[b0, b12, epsilon, I, noise]})],
